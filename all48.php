...
//new
echo "<center><form action='all48.php' method='POST' enctype='multipart/form-data'>
 <table width=90% style='border: 1px solid blue;margin-top:20px;background-color:white;'><tr><td>
 
 </td><td><center><h2><p><b>Завантажити файл з 48 питаннями </b></p></h2></center>
...
foreach ($variantArray as $vk=>$vv)
{
	...
	$myArray=$data->fileToMasAll48($vk, $fileXLS);
...
//echo $tempUserArr[1]." ".$tempUserArr[2]." ".$tempUserArr[3]." ".$tempUserArr[4]." ".$tempUserArr[5]." ".$answerCorrect."<br>";
		if ($group != 0)
		{
		...
		//if (($tempUserArr[1]+$tempUserArr[2]+$tempUserArr[3]+$tempUserArr[4]+$tempUserArr[5])!=0){
...
for($k=1;$k<$data->predmet;$k++){
	...
	$strResult2 .= "<tr><td>Предмет: " . $k . " - Питань: " . $numQueation . " - Легкі: " . $procentLegkuh[$k] . "(" . $lehki[$k] . ")" . "% - Важкі: " . $procentVashkuh[$k] . "(" . $vashki[$k] . ")" . "% - Не валідні: " . $suma[$k] . "% <br />";
	}
}	
